...
/*
    ...
    - RESULT SET : SELECT 구문을 통해 조회된 데이터의 결과무을 의미
                   즉, 조회된 행들의 집합        
    [표현법]
...
/* 
    <컬럼값을 통한 산술연산>
    조회하고자 하는 칼럼들을 나열하는 SELECT절에서 산술(+-/*)를 기술해서 결과를 조회할 수 있다.
*/

-- EMPLOYEE테이블로부터 직원명과, 월급, 연봉
SELECT EMP_NAME, SALARY, SALARY*12 FROM EMPLOYEE;

-- EMPLOYEE테이블로부터 직원명과 월급 보너스, 보너스가 포함된 연봉((SALARY*SALARY*BONUS)*12)
...
-- 결과값이 지저분한 이유 : DATE타입 안에 포함된 시, 분, 초 에 대한 연산까지 수행하기 때문.
-- 결과값은 일수 단위로 출력.

/*
    <컬럼명에 별칭 부여하기>
    [표현법]
    컬렴명 AS 별칭, 컬럼명 AS "별칭", 컬럼명 별칭, 컬럼명 "별칭"
    
    AS를 키워드를 붙이든 안 붙이든 간에 별칭에 특수문자나 띄어쓰기가 포함될 경우 ""를 묶어서 표기해줘야 함.
*/

-- EMPLOYEE테이블로부터 직원명과, 월급, 연봉
SELECT EMP_NAME, SALARY "급여(월)", SALARY*12 AS "연봉(보너스 미포함)" FROM EMPLOYEE;

-- EMPLOYEE 테이블로부터 직원명, 입사일, 근무일수(오늘날짜 - 입사일) 조회
-- DATE타입끼리도 연산이 가능 ( DATE => 년, 월, 일, 시, 분, 초)
-- 오늘 날짜 : SYSDATE
SELECT EMP_NAME AS "사원명", HIRE_DATE AS "입사일", SYSDATE-HIRE_DATE AS "근무일수" FROM EMPLOYEE;
...
/*
    ...
    [표현법]
    DISTINCT 칼럼명
    (단, SELECT 절에 DISTINCT 구문은 단 한 개만 가능하다.)
*/
...
-- EMPLOYEE 테이블로부터 부서코드가 D9인 아닌 사원들의 사원명, 부서코드, 급여조회
...
-- 3. EMPLOYEE 테이블에서 현재 재직중인 사원들의 사번, 이름, 입사일 조회
    ...
    SELECT EMP_ID, EMP_NAME, HIRE_DATE FROM EMPLOYEE WHERE ENT_DATE IS NULL;
...
SELECT C.* FROM ( SELECT EMP_NAME SALARY, SALARY*12 AS 연봉, HIRE_DATE FROM EMPLOYEE) C WHERE 연봉 >= 50000000;
...
-- 부서코드가 D6이거나 급여 300만원 이상인 사원들의 이름, 부서코드, 급여 조회
    SELECT EMP_NAME, DEPT_CODE, SALARY FROM EMPLOYEE WHERE DEPT_CODE = 'D6' OR SALARY >= 3000000;
-- 부서코드가 D6이거나 급여 300만원 이상이면서 재직중인 사원들의 이름 사원들의 이름, 부서코드, 급여 조회
    SELECT EMP_NAME, DEPT_CODE, SALARY FROM EMPLOYEE WHERE (DEPT_CODE = 'D6' OR SALARY >= 3000000) AND ENT_YN = 'N';
-- 급여가 350만원 이상이고 600만원 이하인 사원들의 이름, 사번, 급여, 직급코드 조회
    SELECT EMP_NAME, EMP_ID, SALARY, JOB_CODE FROM EMPLOYEE WHERE 3500000 <= SALARY AND SALARY <= 6000000;
/*
    <BETWEEN AND>
...
-- 입시일이 '90/01/01' ~ '03/01/01'인 사원들의 모든 컬럼 조회;
    SELECT * FROM EMPLOYEE WHERE hire_date BETWEEN '90/01/01' AND '03/01/01';
...
/*
    ...
    '%' : 0글자 이상
          비교대상 칼럼명 LIKE '문자%' => 칼럼값 중에 '문자'로 시작하는 것을 조회
          비교대상 칼럼명 LIKE '%문자' => 칼럼값 중에 '문자'로 끝나는 것을 조회
          비교대상 칼럼명 LIKE '%문자%' => 칼럼값 중에 '문자'가 포함되는 것을 조회
    '_' : 1글자
          비교대상 칼럼명 LIKE '_문자' => 칼럼값 중에 '문자'앞에 무조건 1글자가 존재하는 경우 조회
          비교대상 칼럼명 LIKE '__문자' => 칼럼값 중에 '문자'앞에 무조건 2글자가 존재하는 경우 조회          
*/

-- 성이 전씨인 사원들의 이름, 급여, 입시일을 조회
...
-- 직급코드가 J1, J2, J4가 아닌 사원들의 모든 칼럼 조회
    SELECT * FROM EMPLOYEE WHERE JOB_CODE NOT IN('J1', 'J2', 'J4');
...
SELECT EMP_ID || EMP_NAME || SALARY AS 연결 FROM EMPLOYEE;
...
-- 보너스 기준 정렬
    SELECT * FROM EMPLOYEE
    -- ORDER BY BONUS; -- ASC(오름차순) && NULLS LAT 두개가 기본값.
    -- ORDER BY BONUS ASC NULLS FIRST;
    -- ORDER BY BONUS DESC; -- NULLS FIRST 기본값
    ORDER BY BONUS DESC, SALARY ASC;
 --> 첫 번쨰로 제시한 정렬기준 컬럼값이 일치할 경우 두 번째 정렬기준을 가지고 다시 정렬.
    
-- 연봉기준 줄 세우기
    SELECT EMP_NAME, SALARY, SALARY*12 AS 연봉 FROM EMPLOYEE
    -- ORDER BY 연봉 DESC;
    -- ORDER BY (SALAARY*12) DESC;
    ORDER BY 3;
    
--> ORDER BY 절은 숫자뿐만 아니라 문자열, 날짜에 대해서도 정렬 가능하다.
