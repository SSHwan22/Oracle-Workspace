/*
    ...
    - 실행부 (EXECUTBLE SECTION) : BEGIN으로 시작, SQL문 또는 제어문 등의 로직을 기술하는 부분
    - [예외처리부 (EXCEPTION SECTION) ] : EXCEPTION로 시작, 예외발생 시 해결하기 위한 구문을 미리 기술하는 부분
*/
-- * 화면에 HELLO ORACLE출력해보기
-- 1) 서버 아웃풋 옵션을 켜줌
SET SERVEROUTPUT ON;

BEGIN  
    DBMS_OUTPUT.PUT_LINE('HELLO ORACLE'); 
END; /*밑에 /가 없으면 END; 뒤에 -- 쓰지 말자 오류날수도 있고 안 날수도 있고*/
/

/*
    1. DECLARE 변수나 상수를 선언하는 공간(선언과 동시에 초기화도 가능함.)
    일반타입 변수, 레퍼런스 변수, ROW타입 변수
    
    1_1) 일반타입변수 선언 및 초기화
    [표현식] 변수명 [CONSTANT]자료형 [:=값(대입연산)];ㄴ
*/
DECLARE
    EID NUMBER;
    ENAME VARCHAR2(20);
...
BEGIN
    ...
    -- 보너스가 NULL인경우 빈 값으로 표시됨. NVL함수를 이용해서 10으로 표시해주기
...
/*
    ...
    단, 보너스 받지 않는 사원은 보너율 출력전 '보너스를 지급받지 않는 사원입니다'를 출력
*/
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
...
DECLARE
    ...
    -- NATIONAL 예약어가 있기에 앞에 계정명.테이블명.컬럼명%TYPE으로 명시해줘야 한다.
    TEAM VARCHAR2(10);
BEGIN
...
-- 3) IF 조건식1 THEN 실행내용1 ELSEIF 조건식 2 THEN 실행내용2.... [ELSE] END IF;
-- 급여가 500만원 이상이면 고급
-- 급여가 300만원 이상이면 중급
-- 그 외는 초급
...
--    GRADE :=(CASE WHEN SAL>=5000000 THEN '고급'
--                  WHEN SAL>=3000000 THEN '중급'
--                  ELSE '초급'
--             END);
