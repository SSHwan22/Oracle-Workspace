...
/*12. 학번이 A112113인 김고운 학생의 년도 별 평점을 구하는 SQL 문을 작성하시오. 단, 이 때
 ...
 한 자리까지만 표시한다.*/
SELECT EXTRACT(YEAR FROM TO_DATE(TERM_NO, 'YYYY/MM')) AS 년도, AVG(POINT) FROM TB_GRADE
...
SELECT DEPARTMENT_NO, COUNT(DECODE(ABSENCE_YN, 'Y', 1)) FROM TB_STUDENT
...
SELECT EXTRACT(YEAR FROM TO_DATE(TERM_NO, 'YYYY/MM')) 년도, TO_CHAR(TO_DATE(TERM_NO, 'YYYY/MM'), 'MM') 학기, ROUND(AVG(POINT), 1)
FROM TB_GRADE
WHERE STUDENT_NO = 'A112113'
