...
WHERE DEPT_CODE = DEPT_ID; -- EMPLOYEE테이블의 DEPT_CODE값과 DEPARTMENT(테이블의 DEPT_ID이 일치하는 경우에만 조회.)
...
FROM EMPLOYEE, JOB
WHERE JOB_CODE = JOB_CODE; -- 에러발생 : column ambiguously defined : 컬럼명이 애매모호하다. -> 확실히 어디 테이블의 칼럼인지를 제시해줘야 한다.
...
-- 그 뒤에 JOIN절에서 같이 조회하고자 하는 테이블 기술. 또한 매칭시킬 커럼에 대한 조건도 같이 기술.
-- USING / ON 구문

-- 전체 사원들의 사번, 사원명, 부서코드, 부서명을 알아내고자 할 때
-- 1) 연결할 컬럼명 이 다른 경우( EMLOYEE - DEPT_CODE / DEPARTMENT - DEPT_ID)
--      => 무조건 ON구문만 가능.
SELECT EMP_ID, EMP_NAME, DEPT_CODE, DEPT_TITLE
FROM EMPLOYEE
...
--   => ON, USING구문 둘 다 사용가능하다.
-- 2-1) ON구문 : ambiguously가 발생할 수 있기 때문에 칼럼명을 명확하게 기술해줘야 함.
SELECT EMP_ID, EMP_NAME, E.JOB_CODE, JOB_NAME
FROM EMPLOYEE E
...
FROM EMPLOYEE
JOIN DEPARTMENT ON DEPT_ID = DEPT_CODE;
-- DEPT_CODE가 NULL인 사원 두 명은 조회되지 않음. 즉, "모든" 사원들의 저어보가 출력되는게 아님.

/*
    2. 포괄조인, 외부조인 ( OUTER JOIN)
    테이블 간의 JOIN시에 "일치하지 않는 행도" 포함시켜서 조회가능
...
-- 카테시안의 곱의 경우 : WHERE 조건절에 기술하는 조인문이 잘못되었거나 아예 없을떄 발생.

/*
    4. 비등가 조인(NON EQUAL JOIN)
    '='를 사용하지 않는 조건문 => 다른 비교 연산자를 써서 조인함.(>, <, >=, <=, BETWEEN A AND B)
    => 지정한 칼럼값들이 일치하는 경우 아니라 "범위"에 포함되는 경우 매칭해서 조회하겠다.
