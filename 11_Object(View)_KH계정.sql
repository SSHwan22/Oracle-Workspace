/*
    VIEW : 가상의 테이블(RESULTSSET) SELECT문을 이용해서 실제 테이블에서 가져와서 사용
    VIEW : ORCLE OBJECT로 DDL구문을 이용해서 생성, 수정, 삭제
...
CREATE OR REPLACE VIEW V_EMPLOYEEALL
...
-- 뷰 특징
-- 1. 컬럼에 대해 산술연산 및 함수의 실행결과를 포함할 수 있다. 단, 반드시 별칭을 부여해야 한다.
...
/*
    ...
    만약에 내가 업데이트 하과자하는 칼럼이 가상칼럼이라면 DML이 불가능함.
*/
...
/*
    ...
    3. 산술연산자로 구성된 칼럼(가상칼럼)
    4. 그룹함수, GROUP BY 절이 포함된 VIEW
    5. DISTINCT를 포함하고 있는 경우
    6. JOIN를 통해 여러 테이블을 연결하고 있는 경우
*/
