/*
    ...
    EX)
    회원탈퇴시 기존의 회원테이블에 데이터 DELETE후 곧바로 탈퇴된 회원들만 따로 보관하는 테이블에 자동으로  INSERT처리해야 된다.
    신고횟수가 일정 수를 넘었을 때 묵시적으로 해당 회원을 블랙리스트 처리되게끔 한다.
    입출고에 대한 데이터가 기록될 때마다(INSERT될 때마다) 해당 상품에 대한 제고수량을 매번 수정(UPDATE)해야 도리때 등.
    
    * 트리거 종류
    SQL문의 시행시기에 따른 분류
    > BEFORE TRIGGER : 내가 지정한 테이블에 이벤트(INSERT, UPDATE, DELETE)가 발생되기 전에 트릭거실행
    > AFTER  TRIGGER : 내가 지정한 테이블에 이벤트가 발생된 후에 트리거실행.
    
    SQL문에 의해 영향을 받는 각 행에 따른 분류
    > STATEMENT TRIGEGR(문장트리거) : 이벤트가 발생한 SQL문에 대해 딱 한번만 트리거 실행
...
-- 2. 상품번호 중복 안되게끔 매번 새로운 번호를 발생시키는 시퀀스
...
--   어떤 상품이 어떤 날짜에 몇 개가 입고, 출고가 되었는지 정보를 기록
...
-- 이력번호 매번 새로운 번호를 발생시켜 들어갈 수 있게 도와주는 시퀀스
...
-- TB_PRODUCT테이블에서 200번 상품 10개를 증가시키는 UPDATE문 실행
UPDATE TB_PRODUCT
SET STOCK = STOCK+10
WHERE PCODE = 200;
...
/*
    ...
    - 상품이 입고된 경우 -> 해당상품을 찾아서 재고수량을 증가 UPDATE +
    
    - 상품이 출고된 경우 -> 해당상품을 찾아서 재고수량 감소 UPDATE -
*/
...
BEGIN
    ...
    IF (:NEW.STATUS = '입고')
        THEN 
        UPDATE TB_PRODUCT SET STOCK = STOCK + :NEW.AMOUNT -- NEW키워드활용해서 TB_PRODETAIL에 인서트된 값을 활용할 수 있다.
        WHERE PCODE = :NEW.PCODE;
    ELSE
    -- 상품이 출고된 경우 -> 재고수량을 감소시켜줘야 한다.
        UPDATE TB_PRODUCT
...
INSERT INTO TB_PRODETAIL
VALUES(SEQ_DCODE.NEXTVAL, 210, SYSDATE, 90, '입고');
...
/*
    트리거 장점
    1. 데이터 추가, 수정, 삭제 시 자동으로 데이터관리를 해줌으로써 무결성보장
    2. 데이터베이스 관리의 자동화
    
    트리거 단점
    1. 빈번한 추가, 수정, 삭제시 ROW의 삽입, 추가, 삭제가 함께 실행 되므로 성능상 좋지 못함.
