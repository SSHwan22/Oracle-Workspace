/*
    ...
    만약에 목차가 없다면, 내가 원하는 챕터, 페이지가 어디에 있는 책을 하나하나 흝어 봐야함.
    
    마찬가지로 테이블에서 JOIN 혹은 서브쿼리로 데이터를 조회할 때
    인덱스가 없다면 테이블의 모든 데이터(FULL-SCAN)를 하나하나 뒤져서 내가 원하는 데이터를 가져올 것.
    
    따라서 인덱스 설정을 해두면 모든 테이블의 행들을 뒤지지 않고 내가 웒나느 조건만 빠르게 가져올 수 있다.
...
WHERE EMP_NAME = '선동일';
-- 우리가 인덱스 생성했다고 해서, 그 인덱스를 활용할지 말지는 옵티마이저 판단을 하게됨.
...
/*
    인덱스를 효율적으로 쓰기 위한 방법.
    데이터의 분포도가 높고, 조건절에 자주호출되며, 중복값이 적은 칼럼 => PK칼럼
    ...
    3) MIN, MAX등의 집계함수로 값을 찾을 때 연산속도가 매우 빠름.(정렬되어있기때문에)
    
    인덱스의 단점
    1) DML에 취약함.
       INSERT, UPDATE, DELETE등 데이터가 새롭게 추가, 삭제가 되면 인덱스테이블 안에 있는 값들도 다시 정렬하고,
    ...
    3) 인덱스가 많을수록 저장공간을 잡아먹음.
       인덱스를 만들면 만들수록 저장고간이 부족해지게 되므로, 적절한 수준을 유지해야한다.
*/
