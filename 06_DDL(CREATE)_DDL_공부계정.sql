...
/*
    ...
    <자료형 종류>
    - 문자 (CHAR(크기)/ VARCHAR2(크기)) : 크기는 BYTE수 이다.
                                        (숫자, 영문자, 특수문자 => 1글자당 1BYTE)
      CHAR(바이트 수) : 최대 2000BYTE까지 지정가능.
                       고정길이(아무리 적은 값이 들어와도 빈 공간은 공백으로 채워서 처음 할당한 크기를 유지함
                       주로 들어 올 값의 글자 수가 정해져 있을 경우 사용
                       예) 성별 : 남/여, M/F
                           주민번호 : 6-7 -> 14BYTE                      
      VARCHAR2(바이트 수/CHAR 크기) : 최대길이 4000BYTE까지 가능
                                    가변길이 (적은 값이 들어온 경우 그 담긴 값에 맞춰 크기가 줄어든다)
                                    VAR는 가변 2는 2배를 의미함.
                                    주로 들어온 값의 글자 수가 정해져 있지 않은 경우 사용.
                                    매개변수에 CHAR가 들어온 경우 BYTE단위로 데이터 체크하지 않고 문자의 갯수로 체크함
                                    VARCHAR2(CHAR 10) -> 10글자
      NVARCHAR : 문자열의 바이트가 아닌, 문자 갯수 자체를 길이로 취급ㅂ하여 유니코드를 지원하기 위한 자료형
...
COMMENT ON COLUMN MEMBER.MEMBER_PASSWORD IS '회원비밀번호';
...
-- 유효한 데이터값을 유지하기 위해서 제약조건을 추가해야 한다.
...
/*
    ...
    => 즉, NULL값이 절대 들어와서는 안되는 칼럼에 부여하는 제약조건.
       삽입/수정시 NULL값이 허용하지 않도록하는 제한하는 제약조건
...
INSERT INTO MEM_NOTNULL VALUES(2, 'user01','pass01','민경민', null, null, null);
...
/*
    2. UNIQUE 제약조건
        칼럼에 중복값을 제한하는 제약조건
        삽입 / 수정 시 기존에 해당 컬럼값에 중복된 값이 있을경우 추가 또는 삭제가 되지않게 제약
...
-- unique 제약조건에 위배됨.
-- 제약조건 부여 시 직접 제약조건의 이름을 지정해주지 않으면 시스템에서 알아서 임의의 제약조건명을 부여함
-- SYS_C---(고유한, 중복되지 않은 이름으로 지정)
/*
    제약조건 부여 시 제약조건 명도 지정하는 방법.
    > 칼럼레벨방식
    CREATE TABLE 테이블명 (
        컬럼명 자료형 제약조건1 제약조건2,
...
CREATE TABLE MEM_UNIQUE(
    MEM_NO NUMBER NOT NULL,
    MEM_ID VARCHAR2(20)  NOT NULL,
    MEM_PWD VARCHAR2(20) UNIQUE NOT NULL,
    MEM_NAME VARCHAR2(20) CONSTRAINT MEMBERNAME_NN NOT NULL,
...
-- NULL값을 추가적으로 못 들어오게 하고 싶다면? NOT NULL 제약조건을 걸워줘야 한다.
...
CREATE TABLE MEM_CHECK(
    ...
    PHONE VARCHAR2(15),
    EMAIL VARCHAR2(30),
    MEM_DATE DATE DEFAULT SYSDATE NOT NULL -- DEFAULT설정을 먼저 하고나서 제약조건을 추가해야한다.
    );
...
/*
    INSERT INTO MEM_CHECK
    VALUES (값들 나열); -- 모든 카럼의 값을 직접 제시해줘야함.
    
    INSERT NFO MEM_CHECK(추가할 칼럼명들 나열)
...
/*
    ...
    => 다른 테이블을 참조한다 라고 표현
       ...
       EX) KH계정에서
        ...
        -> DEPT_CODE에는 DEPARTMENT테이블의 DEPT_ID에 존재하는 값만 들어올 숭 있다.(NULL값도 가능)
    ...
    참조할 테이블 == 부모테이블
    생략가능한 것 : CONTRAINT 제약조건명, 참조할 컬럼명.
...
INSERT INTO MEM_GRADE VALUES ('G3', '운영자');
...
WHERE GRADE_CODE = 'G1'; 
-- 자식테이블에서(MEM)에서 GRADE_ID값이 G1인 녀석이 이미 존재하고 있기때문에 함부로 삭제할 수 없다.
...
/*
    ...
    부모테이블의 데이터가 삭제되었을때 자식테이블에는 어떻게 처리할 지를 옵션으로 정해 둘 수 있다.
...
/*
    ...
    다만, 두 컬럼에 동일한 의미의 데이터가 담겨있어야함. (자료형이 같아야 되고, 담긴 값의 종류, 의미도 비슷해야함)
*/
/*
    ------- 접속계정 kh로 변경하기 -------
...
-- EMPLOYEE테이블에 있는 칼럼의 구조만 복사하고 싶을 때 사용하는 방법.
SELECT * FROM EMPLOYEE WHERE 1 = 0; --의도적으로 조건의 결과를 FALSE
