/*
    ...
    [
        ...
        * 캐시메모리?
        ...
        단, 접속이 끊기고 나서 재 접속 후 기존에 생성되어 있던 값들은 제거됨.
    ]
*/
CREATE SEQUENCE SEQ_TEST;

-- 시권에 대한 정보를 조회할 데이터 딕셔너리호출
...
/*
    2. 시퀀스 사용 구문
    시퀀명.CURRVAL : 현재 시퀀스의 값 (마지막으로 성공적으로 발생된 NEXTVAL값)
    시퀀명.NEXTVAL : 현재 시퀀스의 값을 증가시키고, 그 증가된 시퀀스의 값 == 시퀀스명.CURRVAL+INCREMENT_BY로 증가된 값
    
    단, 시퀀스 생성 후 첫 NEXTVAL은 START WITH으로 지정된 시작 값으로 발생 따라서 시퀀스 생성 후 첫 CURRVAL은 수행이 발가능함.
*/
...
SELECT * FROM USER_SEQUENCES;
-- LAST NUMBER : 현재 시퀀에 아픙로 NEXTVAL을 수행하면 반환시킬 값.
...
/*
    ...
    ALTER SEQUENCE 시퀀스명
    -- START WITH 시작숫자      => 처음 발생시킬 시작값 (DEFAULT 1)
    -- START WITH은 변경이 불가능함 : START WITH을 바꾸고 싶다면 시퀀스를 삭제했다가 재 생성해줘야 한다.
...
-- 시작은 300, 증가값은 1, 최대값은 400으로 설정
CREATE SEQUENCE SEQ_EID START WITH 300;

-- 사변이 추가될 때 실행할 INSERT문 작성 사번칼럼에 시퀀스를 이용하여 데이터 추가
...
/*
    사용할 수 없는 구문
    1. VIEW의 SELECT 구문
    2. DISTINCE포함된 SELECT문
    3. GROUP BY HAVING ORDER BY가 있는 SELECT문
    4. SELECT, DELTE, UPDATE 서브쿼리문 안
