/*
    ...
    필요할 때마다 내가 작상한 PL/SQL문을 편하게 호출 가능하다.
...
-- 프로시저에 매개변수를 선언
-- IN : 포르시저 실행 시 필요한 값을 받는 변수( 자바에서의 매개변수와 동일)
...
/*
    ...
    EX) DB에서 대용량의 데이터를 SELECT문으로 받아와서 자바에서 처리하는 경우 VS
        ...
        DB에서 처리하는게 당연히 성능이 더 좋음. (데이터를 넘길 때마다 네트워크자원을 소비하게 됨)
    ...
    정리)
    한번에 처리되는 데이터량이 많고 성능을 요구하는 처리는 대ㅐ체로 자바보다는 DB상에서 처리하는 것이
