...
-- 각 부서별 최고급여를 받는 사원들 회(사번, 이름, 부서코드, 급여)
...
-- 안라인 뷰 사용 : 사원명만 골라내기 (보너스포함연봉이 3000만원 이상인 사원들의 이름만)
SELECT EMP_NAME, 보너스포함연봉
FROM (SELECT EMP_ID, EMP_NAME, (SALARY+SALARY*NVL(BONUS, 0))*12 보너스포함연봉, DEPT_CODE FROM EMPLOYEE
WHERE (SALARY+SALARY*NVL(BONUS, 0))*12 >= 30000000);

-- 인라인 뷰를 주로 사용하는 예
-- TOP - N 분석 : 데이터베이스상에 있는 자료 중 최상위 N개의 자료를 보기위해 사용하는 기능.
...
-- 선 정렬 후 순위 매기기를 해야한다. => 우선적으로 인라인뷰로 ORDER BY절을 만들고 그 후 에 메인쿼리에서 순번을 붙임.

-- 가장 최근에 입사한 사원 5명 (사원명, 급여, 입사일)
SELECT ROWNUM, E.*
FROM (SELECT HIRE_DATE, SALARY, EMP_NAME FROM EMPLOYEE ORDER BY HIRE_DATE DESC) E
WHERE ROWNUM <=5;
...
/*
    ...
    - RANK() OVER(정렬기준) : 공동 1위가 3명이라면 한다면 그 다음 순위는 4위로.
    - DENSE_RANK() OVER(정렬기준) : 공동 1위가 3명이면 그 다음 순위는 2위로.
    
    정렬 기준 : ORDER BY절(정렬기준 컬럼이름, 오름차순/내림차순), NULLFIRST나 NULLSLAST는 기술불가.
