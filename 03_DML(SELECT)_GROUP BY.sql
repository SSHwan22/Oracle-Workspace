...
*/
 ...
 /*
    ...
    - UNION ALL : 두 쿼리문을 수행한 결과 값을 더한 후 중복 제거를 하지 않은 것 => 합집합 + 교집합
    - INTERSECT(교집합) : 두 쿼리문을 수행한 결과값의 중복된 부분 => AND
    - MINUS (차집합) : 선행 쿼리문 결과값에서 후행 쿼리문 결과값을 뺀 나머지 부분
                       => 선행 쿼리문 결과값 - 교집합
    
    주의해야할 점 : 두 쿼리문의 결과를 합쳐서 한 개의 테이블로 보여줘야 하기 때문에 두 쿼리문의 SELECT절 부분은 같아야 한다.
                  즉 조회할 "컬럼"이 동일해야 한다.
 */
...
-- 2. UNION ALL : 여러 개의 쿼리결과를 더해서 보여주는 연산자. (중복제거 안함.)
...
-- 3. INTERSECT : 교집합, 여러 쿼리 결과의 중보된 결과만 조회 => AND
-- 직급코드가 J6이거나 부서코드가 D1인 사원들을 조회(사번, 사원명, 부서코드, 직급코드)
SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE
FROM EMPLOYEE
WHERE JOB_CODE = 'J6'
