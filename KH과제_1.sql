...
(SALARY+SALARY*NVL(1+BONUS, 0))*12 AS 총수령액,
(SALARY+SALARY*NVL(1+BONUS, 0))*12 - ((SALARY*12) * 0.3)AS 실수령액
FROM EMPLOYEE;
...
HIRE_DATE
FROM EMPLOYEE
WHERE (SALARY+SALARY*NVL(1+BONUS, 0))*12 - ((SALARY*12) * 0.3) >= 50000000;
...
SELECT EMP_NAME, DEPT_CODE, HIRE_DATE FROM EMPLOYEE WHERE DEPT_CODE = 'D9' OR DEPT_CODE ='D5' AND HIRE_DATE <= TO_DATE('020101');
...
WHERE EMAIL LIKE '____!_%' ESCAPE '!' AND DEPT_CODE IN('D6','D9') AND HIRE_DATE BETWEEN '90/01/01' AND '010101' AND SALARY >= 2700000;

--15. EMPLOYEE테이블에서 사원 명과 직원의 주민번호를 이용하여 생년, 생월, 생일 조회
SELECT EMP_NAME, TO_CHAR(TO_DATE(SUBSTR(EMP_NO,1,6)),'RRRR')||'년' 생년, SUBSTR(EMP_NO, 3,2)||'월' 생월, SUBSTR(EMP_NO, 4, 2)||'일' 생일   FROM EMPLOYEE;
...
SELECT EMP_NAME, ABS(ROUND(HIRE_DATE-SYSDATE)) 근무일수1, ROUND(SYSDATE-HIRE_DATE) 근무일수2 FROM EMPLOYEE;
...
--TO_CHAR(SYSDATE, 'RRRR') - EXTRACT( YEAR FROM TO_DATE(SUBSTR(EMP_NO, 1,6))) 나이
...
-- (단, 부서코드 오름차순으로 정렬)
SELECT * FROM EMPLOYEE WHERE DEPT_CODE IN('D5','D6','D9') ORDER BY DEPT_CODE;

--23. EMPLOYEE테이블에서 사번이 201번인 사원명, 주민번호 앞자리, 주민번호 뒷자리, 
-- 주민번호 앞자리와 뒷자리의 합 조회
SELECT EMP_NAME, SUBSTR(EMP_NO, 1, 6) AS "주민번호 앞자리", SUBSTR(EMP_NO, 8,14)AS "주민번호 뒷자리", SUBSTR(EMP_NO, 1, 6) + SUBSTR(EMP_NO, 8,14) AS "주민번호 합" FROM EMPLOYEE;

--24. EMPLOYEE테이블에서 부서코드가 D5인 직원의 보너스 포함 연봉 합 조회
SELECT SUM(SALARY * (1+BONUS)*12) AS "D5 연봉 합(보너스포함)" FROM EMPLOYEE WHERE DEPT_CODE = 'D5' GROUP BY DEPT_CODE; 
...
COUNT( DECODE(EXTRACT(YEAR FROM HIRE_DATE), 2002, 'r', NULL)) AS "2002",
COUNT( DECODE(EXTRACT(YEAR FROM HIRE_DATE), 2003, '가', NULL)) AS "2003",
COUNT( DECODE(EXTRACT(YEAR FROM HIRE_DATE), 2004, 1234, NULL)) AS "2004"
FROM EMPLOYEE 
